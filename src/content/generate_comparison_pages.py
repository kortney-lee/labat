...
from src.content.commercial_page_policy import (
    ...
    comparison_reference_block,
    sanitize_comparison_post,
)

logger = logging.getLogger("wihy.generate_comparison_pages")
logging.basicConfig(level=logging.INFO, format="%(levelname)s | %(message)s")

OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "")
MODEL = "gpt-4o"
INVENTORY_FILE = Path("data/communitygroceries_comparison_pages.json")
PROGRESS_FILE = Path("data/communitygroceries_comparison_progress.json")
...
def _expand_short_body(body: str, page: Dict[str, Any], keyword: str) -> str:
    if not body or len(body.split()) >= MIN_WORD_COUNT:
        return body

    title = page.get("title", keyword.replace("-", " ").title())
    bridge = (
        f"\n\nFor shoppers researching {keyword}, the decision usually comes down to how much structure they want versus how much flexibility they need week to week. "
        f"{title} is not just a pricing question. It is also about whether you want pre-selected meals, how often you need to swap ingredients, and whether your household can realistically stay consistent with the format over a full month.\n\n"
        "A practical way to compare the options is to think through one normal week: how many dinners you need covered, whether leftovers matter for lunch, how often tastes change inside the household, and what happens when produce prices or schedules shift. "
        "That week-to-week flexibility is often what separates a short-term trial from a routine you can actually sustain."
    )

    faq_index = body.lower().find("## faq")
    if faq_index != -1:
        return body[:faq_index].rstrip() + bridge + "\n\n" + body[faq_index:].lstrip()
    return body.rstrip() + bridge
...
def _validate_payload(payload: Dict[str, Any], keyword: str) -> List[str]:
    ...
    seo_keywords = [str(item).strip().lower() for item in payload.get("seo_keywords", []) if str(item).strip()]
    ...
    if keyword.lower() not in title_lower and not any(term in title_lower for term in seo_keywords):
    ...
    if citations:
        citation_check = validate_citations(citations, citation_type="comparison")
...
SYSTEM_PROMPT = """You are Kortney writing high-conviction comparison pages for Community Groceries.

These are not generic blog posts. They are commercial-intent decision pages for people comparing meal kits, prepared meal services, and grocery delivery.

Write with these principles:
- Be direct and useful.
- Do not sound like an ad.
- Acknowledge where the competitor is strong.
- Explain where Community Groceries is better for flexibility, healthier choices, and long-term value.
- Avoid unsupported factual claims about competitor pricing, features, or nutrition.
- If evidence is limited, say so clearly.
- Community Groceries costs $9.99 per month. Never say it is free or that it does not require a subscription.
- Do not claim a fixed savings number for Community Groceries unless the exact figure is supported by a citation included in the output.
- If competitor pricing is not verified in an official source, do not present a hard dollar figure as settled fact. Say pricing varies by plan size, promos, shipping, and add-ons.
- When comparing affordability, distinguish Community Groceries membership cost from the household's grocery spend.
- Do not use PubMed-style or unrelated academic citations for these commercial pages. Use only the official consumer guidance references provided.

Use these exact markdown headings in the body:
- Quick answer
- Head-to-head comparison
- Cost and value
- Health / nutrition / flexibility
...
{{
    "slug": "{slug}",
    "title": "SEO title",
    "body": "Markdown article, 900-1400 words",
...
Required structure in body:
1. Quick answer
2. Head-to-head comparison
3. Cost and value
4. Health / nutrition / flexibility
5. Who each option is best for
6. Bottom line
7. FAQ

Length and specificity rules:
- The body must be 900-1400 words.
- Each main section before the FAQ needs at least 2 substantive paragraphs.
- Do not use placeholder copy, generic filler, or broad statements that could fit any brand.
- Anchor the comparison to the specific keyword and competitor named in the prompt.
- If the draft is under the minimum length, expand the analysis instead of shortening sections.

For comparison pages, include a markdown table near the top.
For alternative pages, include a "Why people look for alternatives" section.
For guide pages, stay category-level and practical.

CTA requirement:
End with a concrete CTA to Community Groceries.
"""
...
async def generate_page(
    ...
    page_type = page.get("page_type", "comparison")

    reference_block = comparison_reference_block()
...
Write the page so it can rank for the keyword and help users decide.

Hard requirements:
- The body must land between 900 and 1400 words.
- Every required section must appear as a clear markdown heading.
- The title must include the primary keyword.
- The draft must be specific to this keyword, not reusable generic meal-planning copy.

Known brand facts you must preserve:
- Community Groceries is a $9.99/month subscription.
- Do not say "no subscription", "free", or "no commitment" for Community Groceries.
- Savings should be framed carefully: the subscription may pay for itself if it helps reduce waste, duplicate purchases, or takeout spend, but exact savings vary by household.
- If you cannot verify a competitor's current exact meal price, say the current cost varies instead of inventing a fixed range.

Citation rules for this page type:
- Use only the official references provided below.
- Do not cite PubMed studies, journals, or unrelated medical trials in these comparison, alternative, or cost pages.
- If you include citations, keep them aligned to grocery budgeting, healthy eating guidance, and household food-cost context.
...
"""
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        payload["seo_keywords"] = page.get("seo_keywords", payload.get("seo_keywords", []))
        payload["title"] = page.get("title", payload.get("title", slug.replace("-", " ").title()))
        payload["meta_description"] = page.get("meta_description", payload.get("meta_description", ""))
        ...
        payload.setdefault("tags", [page_type, topic_slug, "comparison-intent"])
        payload["citations"] = comparison_citations()
        payload["body"] = _expand_short_body(str(payload.get("body", "") or ""), page, keyword)
        payload["word_count"] = len(payload["body"].split()) if payload.get("body") else 0
        ...
        if not issues:
            ...
            return sanitize_comparison_post(payload)
        ...
        retry_prompt = (
            ...
            + "\n- ".join(issues)
            + "\n\nReturn a fuller, page-specific draft that satisfies every required section and word-count requirement. Preserve the good parts of the previous draft, but expand the weak sections. If the draft is short, add concrete details to every non-FAQ section until the body is at least 850 words. Keep the required section headings exactly as written in the prompt.\n\nPrevious draft JSON:\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Community Groceries comparison pages")
